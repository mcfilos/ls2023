...
*/
class NKls_rule_exis_intr extends NKls_rule{
		...
		foreach($lines as $l=>$line){
			...
			foreach(array_keys(NKls_formula::$variables_used) as $var){
				...
				foreach($constants as $const){
					$subclones = array();
					$subclones = $line->formula()->clone_replaced([],[$const => $var],true);

				

					foreach($subclones as $subclone){
						//var_dump($subclone->variables(false), $line->formula()->string(),$var);
						$nextf = new NKls_formula($subclone,'E'.$var) ;
